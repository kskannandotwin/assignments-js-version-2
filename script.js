...
// Use whatever loop and methods you want. You can also use the spread syntax. In the end, the allKeywords
// array should look more or less like this: ['computer science', 'programming', 'algorithms',
// 'data structures', ...].
...
// Convert the first book object from the books array into a Map, and assign it
// to a firstBookMap variable.
...
// Take the ISBN property of the first book from the books array, and log to the console
// characters at index 6, 4, 9 and 8. Use bracket notation to access individual
// characters.
...
// Below is the quote variable that stores a string. Find the index of the word
// 'chess', and log it to the console.
...
// and returns the same string, but the first name and last name are capitalized, and the
// "(Contributor)" part is removed (if exists).

// You can be sure that the author's name always consists of two words separated by a space, and
// possibly ends with "(Contributor)". The string may also contain trailing spaces.
...
// Take the title of the second book (books[1]) from the books array, and replace the word
// "Programs" with "Software". Assign the new string to the newBookTitle variable.
...
/*
and, "This book is about some systems, but definitely not about operating systems" if the title
ends with the word "system" or "systems", but doesn't include the word "operating".
*/
...
// Below is the bookCategories variable that stores a string of categories. Each category is separated
// with a semicolon, for example, in a string "science;computing", 'science' and 'computing' are
// separate categories.
...
// Write a function called getKeywordsAsString that takes the books array as an argument, collects keywords
// from each book, removes duplicates, and then joins them to create a single string where keywords
...
// and the number of a page, for example, in ['The Basics', 14], 'The Basics' is the chapter's title, and 14
// is the number of a page.

// Write a function called logBookChapters that takes an array of arrays (like bookChapters) as an argument,
// and logs each chapter's name to the console together with the page number. The page number should be
// separated from the chapter's name with underscores (take a look at the example below).
